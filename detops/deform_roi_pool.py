...
class DeformRoIPoolFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx,
                input,
                rois,
...
class DeformRoIPoolPack(DeformRoIPool):
    ...
    def forward(self, input, rois):
        ...
        roi_nums = rois.size(0)
        offset = self.offset_fc(x.view(roi_nums, -1))
        offset = offset.view(roi_nums, 2, self.output_size[0],
                             self.output_size[1])
        return deform_roi_pool(input, rois, offset, self.output_size,
                               self.spatial_scale, self.sampling_ratio,
                               self.gamma)
